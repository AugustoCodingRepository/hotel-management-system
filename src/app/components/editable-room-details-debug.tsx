...
export const EditableRoomDetailsDebug = forwardRef<RoomDetailsRef, RoomDetailsProps>(({ roomNumber }, ref) => {
  const { roomData, setRoomData, isLoading, isSaving, lastSaved, clearAccount } = useRoomAccountRealtime(roomNumber)

  // Debug: log quando cambia la camera
  useEffect(() => {
    console.log(`🏠 Camera cambiata a: ${roomNumber}`)
    console.log(`📊 Dati camera:`, roomData)
  }, [roomNumber, roomData])

  // Calculate totals from table data
  const roomTotal = useMemo(() => {
    const cameraValues = Object.values(roomData.tableData.Camera || {})
    const total = cameraValues.reduce((sum, value) => sum + (Number.parseFloat(value) || 0), 0)
    console.log(`💰 Room total per camera ${roomNumber}:`, total, cameraValues)
    return total
  }, [roomData.tableData.Camera, roomNumber])
  ...
  const extrasFromTable = useMemo(() => {
    ...
    servicesToSum.forEach((service) => {
      ...
      const serviceTotal = serviceValues.reduce((sum, value) => sum + (Number.parseFloat(value) || 0), 0)
      total += serviceTotal
      if (serviceTotal > 0) {
        console.log(`💰 ${service} total:`, serviceTotal, serviceValues)
      }
    })
  ...
  const updateField = (field: string, value: string | number) => {
    console.log(`📝 Aggiornamento campo ${field}:`, value)
    setRoomData({ [field]: value })
  }

  const updateTableCell = (service: string, date: string, value: string) => {
    console.log(`📝 Aggiornamento tabella ${service}[${date}]:`, value)
    setRoomData({
      tableData: {
        ...roomData.tableData,
        [service]: {
  ...
  const handleAzzeraConto = async () => {
    ...
    if (conferma) {
      console.log(`🗑️ Azzeramento conto camera ${roomNumber}`)
  ...
  const handlePrint = async () => {
    console.log(`🖨️ Stampa conto camera ${roomNumber}`)
  ...
  return (
    <div className="flex-1 p-6 bg-gray-50 relative">
      {/* Debug info */}
      <div className="absolute top-2 left-4 z-10 bg-gray-800 text-white px-2 py-1 rounded text-xs">
        Camera: {roomNumber} | Customer: {roomData.customer || "Vuoto"} | Total: €{finalTotal.toFixed(2)}
      </div>

      {/* Indicatore di salvataggio */}
      <div className="absolute top-4 right-4 z-10">
        {isSaving ? (
          <div className="flex items-center space-x-2 bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-medium">
          ...
          <div className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs">💾 Pronto per salvare</div>
        )}
      </div>

      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-8" data-print-content>
        {/* Header */}
...
EditableRoomDetailsDebug.displayName = "EditableRoomDetailsDebug"
